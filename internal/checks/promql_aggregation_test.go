	...
	testCases := []checkTest{
		{
			...
			checker:     checks.NewAggregationCheck(checks.MustTemplatedRegexp(".+"), "job", true, checks.Warning),
		},
		{
			description: "name must match / recording",
			content:     "- record: foo\n  expr: sum(foo) without(job)\n",
			checker:     checks.NewAggregationCheck(checks.MustTemplatedRegexp("bar"), "job", true, checks.Warning),
		},
		{
		...
		},
		{
			...
			content:     "- record: foo\n  expr: sum(foo) without(instance, job)\n",
			checker:     checks.NewAggregationCheck(checks.MustTemplatedRegexp(".+"), "job", true, checks.Bug),
			problems: []checks.Problem{
				{
					Fragment: "sum(foo) without(instance, job)",
					Lines:    []int{2},
		...
		},
		{
			...
			checker:     checks.NewAggregationCheck(checks.MustTemplatedRegexp(".+"), "job", false, checks.Warning),
			problems: []checks.Problem{
				{
		...
		},
		{
			...
			checker:     checks.NewAggregationCheck(checks.MustTemplatedRegexp(".+"), "instance", false, checks.Warning),
		},
		{
