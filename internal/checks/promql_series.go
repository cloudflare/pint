...
func (c SeriesCheck) Check(ctx context.Context, _ discovery.Path, rule parser.Rule, entries []discovery.Entry) (problems []Problem) {
			...
			if alertname != "" {
				...
				if arEntry != nil {
					...
					problems = append(problems, Problem{
						...
						Reporter: c.Reporter(),
						Summary:  fmt.Sprintf("`%s` metric is generated by alerts but didn't found any rule named `%s`.", selector.String(), alertname),
		...
		if len(trs.Series.Ranges) == 0 {
			...
			if rrEntry != nil {
				...
				problems = append(problems, Problem{
					Lines:    expr.Value.Lines,
					Reporter: c.Reporter(),
					Summary: fmt.Sprintf("%s didn't have any series for `%s` metric in the last %s but found recording rule that generates it, skipping further checks.",
	...
	for _, disable := range orphanedDisableComments(ctx, rule, selectors) {
		problems = append(problems, Problem{
			Lines:    expr.Value.Lines,
			Reporter: c.Reporter(),
			Summary:  fmt.Sprintf("pint %s comment `%s` doesn't match any selector in this query", comments.DisableComment, disable.Match),
	...
	for _, ruleSet := range orphanedRuleSetComments(rule, selectors) {
		problems = append(problems, Problem{
			Lines:    expr.Value.Lines,
			Reporter: c.Reporter(),
			Summary:  fmt.Sprintf("pint %s comment `%s` doesn't match any label matcher in this query", comments.RuleSetComment, ruleSet.Value),
	...
	for _, ruleSet := range comments.Only[comments.RuleSet](rule.Comments, comments.RuleSetType) {
		...
		if err != nil {
			problems = append(problems, Problem{
				Lines:    rule.Lines,
				Reporter: c.Reporter(),
				Summary:  fmt.Sprintf("Failed to parse pint comment as duration: %s", err),
